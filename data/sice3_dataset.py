...
class SICE3Dataset(BaseDataset):
    def __init__(self, opt):
        ...
        for scene in self.scenes_list:
            ...
            self.medium_path.append(os.path.join(self.root_dir, self.medium, scene))
    ...
    def __getitem__(self, index):
        ...
        img_medium = Image.open(self.medium_path[index]).convert('RGB')
        ...
        if (self.opt.mode == 'Test'):
            ...
            img_medium = np.array(img_medium.resize((new_w // 4, new_h // 4), resample=resample_filter))
        ...
        img_under, img_medium, img_over, img_label = self.trans(img_under, img_medium, img_over, img_label)

        return {'under': img_under, 'medium': img_medium, 'over': img_over, 'label': img_label, 'label_path': self.label_path[index]}
