...
import numpy as np
import torch
from .AttnMerger import FusionConfigs
...
class AMMEF2(nn.Module):
    def __init__(self, opt):
        super(AMMEF2, self).__init__()
        filters_in = 3
        filters_out = 3
        nFeat = 32
        ...
        # merge
        self.merger = FusionConfigs.balanced()

        # decoder - 使用 Upsample + Conv (方案2)
        ...
        self.conv_D2_1 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(nFeat * 12, nFeat * 2, 3, 1, 1, bias=True)
        )
        self.conv_D2_2 = Res_block(nFeat * 2)

        self.conv_D1_1 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(nFeat * 6, nFeat, 3, 1, 1, bias=True)
        )
        self.conv_D1_2 = Res_block(nFeat)

        self.conv_D0_1 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(nFeat * 3, nFeat, 3, 1, 1, bias=True)
        )
    ...
    def forward(self, x1, x2):
        ...
        res_tensor = self.merger(E1_3, E2_3)
