...
# it doesn't matter where the long side is
class Channel_Embeddings(nn.Module):
    """Construct the embeddings from patch, position embeddings.
    """
    def __init__(self, opt, patch_size, img_size, in_channels):
        super().__init__()
        ...
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       ...
                                       stride=patch_size)
        # nn.Parameter is to define it as a trainable parameter
        self.position_embeddings = nn.Parameter(torch.zeros(1, n_patches, in_channels))
        self.dropout = Dropout(opt.CT_embedding_dropout)

    def forward(self, x):
        if x is None:
            return None
        # print("x shape before operation: ", x.shape)
        x = self.patch_embeddings(x)  # (B, hidden. n_patches^(1/2), n_patches^(1/2))
        # print("x shape after patch embedding: ", x.shape)
        # Flatten the dimension starting from the second dimension to the last one into one dimension
...
class Attention_org(nn.Module):
    def __init__(self, opt, vis, channel_num):
        ...
        self.num_attention_heads = opt.CT_num_heads
        ...
        for _ in range(opt.CT_num_heads):
            ...
            # deepcopy is to avoid the same object being appended to the list
        ...
        # normalize the values by each head
        # it actually is used to normalize by each channel (the second dimension in the tensor)
        # so here the second dimension is set to the number of attention heads
        ...
        self.attn_dropout = Dropout(opt.CT_attention_dropout)
        self.proj_dropout = Dropout(opt.CT_attention_dropout)
...
class Mlp(nn.Module):
    def __init__(self,opt, in_channel, mlp_channel):
        ...
        self.act_fn = nn.GELU()
        self.dropout = Dropout(opt.CT_transformer_dropout)
        self._init_weights()
...
class Block_ViT(nn.Module):
    def __init__(self, opt, vis, channel_num):
        ...
        self.channel_attn = Attention_org(opt, vis, channel_num)
        ...
        self.ffn1 = Mlp(opt,channel_num[0],channel_num[0]*expand_ratio)
        self.ffn2 = Mlp(opt,channel_num[1],channel_num[1]*expand_ratio)
        self.ffn3 = Mlp(opt,channel_num[2],channel_num[2]*expand_ratio)
        self.ffn4 = Mlp(opt,channel_num[3],channel_num[3]*expand_ratio)
...
class Encoder(nn.Module):
    def __init__(self, opt, vis, channel_num):
        ...
        for _ in range(len(channel_num)):
            layer = Block_ViT(opt, vis, channel_num)
...
class ChannelTransformer(nn.Module):
    def __init__(self, opt, vis, channel_num):
        ...
        image_h, image_w = [opt.downsample * float(i) for i in opt.fineSize.split(',')]
        image_h = int(image_h)
        image_w = int(image_w)
        patch_h = image_h // opt.CT_patch_ratio
        patch_w = image_w // opt.CT_patch_ratio
        patch_size1 = (patch_h, patch_w)
        patch_size2 = (patch_h//2, patch_w//2)
        patch_size3 = (patch_h//4, patch_w//4)
        patch_size4 = (patch_h//8, patch_w//8)
        self.embeddings_1 = Channel_Embeddings(opt, patch_size1, (image_h, image_w), in_channels=channel_num[0])
        self.embeddings_2 = Channel_Embeddings(opt, patch_size2, (image_h//2, image_w//2), in_channels=channel_num[1])
        self.embeddings_3 = Channel_Embeddings(opt, patch_size3, (image_h//4, image_w//4), in_channels=channel_num[2])
        self.embeddings_4 = Channel_Embeddings(opt, patch_size4, (image_h//8, image_w//8), in_channels=channel_num[3])
        self.encoder = Encoder(opt, vis, channel_num)

        self.reconstruct_1 = Reconstruct(channel_num[0], channel_num[0], kernel_size=1,scale_factor=(patch_size1))
        self.reconstruct_2 = Reconstruct(channel_num[1], channel_num[1], kernel_size=1,scale_factor=(patch_size2))
        self.reconstruct_3 = Reconstruct(channel_num[2], channel_num[2], kernel_size=1,scale_factor=(patch_size3))
        self.reconstruct_4 = Reconstruct(channel_num[3], channel_num[3], kernel_size=1,scale_factor=(patch_size4))
